...
`lifetime.ts` constructs one group per selected person's stage, eligible unit, and interval. Every lifetime group requires the same recorded broad subject (`mathematics`, `physics`, `chemistry`, or `biology`), including when matching a student or postdoc with a faculty peer. Missing or unsupported subjects do not match. This is a filter based on the public researcher's roster classification, not an independently verified historical department.

Doctoral groups additionally require overlapping doctoral peers and independently observed/verified faculty at the same school and historical department. Postdoc groups permit doctoral, postdoc, and verified faculty peers only at the same institution and recorded subject during the selected interval. Institution-wide postdoc matches are no longer allowed: for example, physics and mathematics researchers at the Korea Institute for Advanced Study are separated. First-assistant groups require an explicit assistant-professor rank and a reviewed first-appointment flag, then match observed/verified faculty at the same school, historical department and recorded subject. Current groups use the same canonical school and recorded subject in the release year; an older roster is not extended to the present.

`subjectProvenance: 'recorded_researcher_subject'` identifies the field filter on intervals, groups, member evidence and rendered edges. Doctoral/first-assistant groups use `matchingBasis: 'institution_department_subject'`; postdoc/current groups use `matchingBasis: 'institution_subject'`. A roster subject never fills a historical department field or confirms that a postdoc belonged to a specific past department. The postdoc rule operates at broad subject level because the available data do not consistently identify historical departments.

The current stage carries `subject` separately from department evidence. Missing, different, or inferred department text cannot split that group, and the inferred-department toggle does not affect current membership. For example, the ten 2026 physics researchers at Gyeongsang National University previously split into six with `Department of Physics and Research Institute of Natural Science` and four with `Department of Physics`; they now share one current group, from either person's perspective.

A specific current school and source-confirmed release-year employment observation remain necessary. Missing country is retained as missing and does not exclude an otherwise eligible current roster row. Known conflicting countries for the same canonical school are kept apart; unknown country must not bridge such a conflict. Explicit campus distinctions and dated institution-successions remain in force. Doctoral and postdoc country validation is unchanged.

`createLifetimeIndex` prepares profiles and stage/unit candidate indexes once and exposes every valid researcher ID, including researchers with no eligible interval. The full-release directory computes compact summaries in a worker, while selecting a row creates the detailed graph and evidence. Indexing preserves the original per-person group membership, ordering and evidence semantics. See [whole-release career exploration](ALL_PROFESSOR_CAREERS.md) for coverage and computation details.

Faculty employment comes from `faculty_appointments` with semester-roster, official-profile, or verified-CV evidence. A generic `faculty` career stage, career sequence, publication affiliation, or non-estimated flag alone is not employment confirmation. Doctoral windows retain their observed/estimated distinction. The current record uses separate `current_position` evidence and the public roster's subject. Unknown schools, conflicting countries or unverified employment do not create a connection. Department evidence is required only for doctoral and first-assistant matching; publication-derived departments remain explicitly inferred and require opt-in in those stages.
...
Building incidence and each diffusion pass cost `O(total incidence)`. Overlap counting costs `O(sum_v incident_edge_count(v)^2)`, small for bounded doctoral windows. No all-pairs researcher similarity matrix is built. Collision handling uses spatial hashing. Generic trajectory lookup is linear in the number of recorded unit-year observations for one selected researcher. The indexed lifetime path restricts peer comparisons to candidates in the selected stage's institution unit; detailed evidence is materialized only for the requested person.
