...
- Korean institutional units require the recorded degree department as well as school; current appointment department and broad subject are never substitutes. Foreign school matching ignores department. Korean and foreign institutions are distinguished using normalized country codes.
...
Each peer carries matching institution, country, department where relevant, both career stages, shared year interval, and estimated/inferred flags. Unknown units or invalid dates are excluded, and coverage diagnostics identify why. Actual doctoral intervals can use degree metadata only when they refer to the same recorded/canonical doctoral institution. Current faculty department is never imported into degree history. Country-less postdoc/faculty intervals remain excluded.
...
Validation uses strict TypeScript compilation and 22 Node tests: inclusive endpoints, observed-vs-estimated precedence, non-contiguous intervals, missing values, country/department rules, inference opt-in, canonical aliases, duplicate IDs, absence of private fields, cardinality and overlap weighting, layer toggles, trajectory evidence/Jaccard, bachelor estimation, joint degree cohorts, chronological ordering, deterministic disconnected layouts, finite outputs, weight sensitivity, and 3,937-node behavior. A separate actual-data benchmark checks every candidate pair within nine horizontal pixels using an independent x-sweep, rather than trusting the collision implementation's own counter.

Actual enriched 3,937-node data at 1400×1000, minimum distance 9, joint doctoral cohorts and chronological display enabled: approximately 0.62 s with strict departments and 0.54 s with inferred departments enabled on the bundled Node runtime. Both, spatial-only, temporal-only, and empty-edge modes had zero independent final collision pairs. Nodes remained inside the padded bounds. Full-data trajectory matching took about 25 ms for the benchmark selection. Timings are environment-dependent.
