...
2026-09-10. 국내·해외 박사학위자 모두를 대상으로 한다. 교수 요약에 `thesis_url` 하나를 추가하는 대신 기존 비공개 연구자 식별자 DB를 버전 4로 확장해 학위, 논문, 연결 검토와 공식 공개 상세 기록의 관측 자료를 나눈다. 기존 원본 DB와 공개 파일은 수정하지 않는다.
...
| `thesis_records` | `thesis_id` PK, `(provider, provider_record_id)` UNIQUE, 제목·저자·출판연도·출처 URL·DOI | RISS, OpenAlex 및 등록된 공식 대학 도서관의 논문 기록. 공급자가 보고한 학위종류·학과는 별도 컬럼이며 미제공 시 NULL이다. |
...
| `thesis_providers` | 공급자 PK, 출처 종류 | 실제 공식 기록 형식을 지원하는 공급자만 등록한다. 논문의 `provider` FK가 미등록 공급자를 거부한다. |
...
`import-library`는 KAIST의 KOASAS `10203/...` 핸들과 도서관 서지 `bib:...`, 서울대학교 S-Space `10371/...` 핸들, 서울대·연세대·고려대·이화여대·중앙대·성균관대·동국대·숙명여대 dCollection `dcollection:...`, POSTECH 공식 Primo 도서관의 `alma:...`, 경희대 도서관의 `catalog:CATSAZ...` 기록을 지원한다. 공식 HTTPS 상세 URL과 공급자·서지 ID의 일치 여부를 검사한다. 검색 URL, 다른 도메인, 인증 매개변수는 허용하지 않는다. dCollection의 `common/orgView/<12자리 ID>`와 `srch/popup/srchMetaViewPopup/<12자리 ID>`는 같은 ID의 `srch/srchDetail` URL로 표준화한다. POSTECH은 실제 조회된 전용 도메인과 기관 코드만 허용하며 다른 기관의 공용 Alma 주소는 받지 않는다.

후보 입력은 `degree_id`와 원본 스냅샷 해시뿐 아니라 저장된 원본 학위 ID·연구자 ID·이름·기관 ID와 표준명·학위연도·두 국가 코드를 모두 대조한다. 해당 도서관이 학위 수여기관인지뿐 아니라, 실제 학위사항의 `institution_statement`가 저장된 기관과 일치하는지도 별도로 검사한다. 도서관 도메인이나 `publisher`만으로 학위 수여기관을 결정하지 않는다. 등록된 기관 동의어와 일반적인 대학원 접미사만 정규화한다. 실제 자료에서 확인한 전문대학원은 정확한 전체 명칭별로만 대학에 연결하며, 비슷한 이름의 다른 대학·병원·별도 캠퍼스는 합치지 않는다. 표준기관이 등록된 학교인 경우 원본 기관 표기도 같은 동의어 집합에 속해야 하며, 해석되지 않는 원본 표기나 캠퍼스 불일치는 새 입력에서 보류한다. 지도교수와 구별된 실제 저자 필드에 학위자의 이름이 있어야 하며, 명시된 박사학위 문구와 수여연도 ±1년 이내 출판연도를 요구한다. 국가가 충돌하거나 같은 논문이 여러 연구자에게 연결되면 `conflict`로 남긴다. 원문에 기재된 학과는 `reported_department`에 보존하며 검증 완료 학과로 승격하지 않는다.
...
{schema_version: 1, source: "Official university repository", candidates: [{
  ...
  observation: {fetched_at, retrieval_method, source_sha256, record_reference,
                ...
                source_metadata: {degree_statement: 실제 학위 문구,
                                  institution_statement: 실제 학위 수여기관,
                                  ...원문 필드}}
}]}
```
...
`import-riss-public`는 같은 후보 구조에서 `source`를 `RISS public degree details`로 지정하고 `provider`는 `riss`로 사용한다. 실제 공개 상세 페이지의 학위사항·저자·연도를 저장하는 경로로, RISS OpenAPI 요청·응답이나 API 키 사용을 주장하지 않는다. 기존 `import-riss`의 `RISS thesis OpenAPI` 수집 감사 형식과 구분한다. 동일한 RISS ID는 기존 기록을 재사용하며, 다른 공급자의 같은 DOI를 자동 병합하지 않는다.

관측된 수여기관이 등록된 국내 기관에 해당하면 기관 국가를 KR로 판정한다. 그 외 기관은 수집 근거에 `source_metadata.institution_country_code`가 있어야 한다. RISS 도메인만 보고 국내 학위로 정하지 않는다. 이 관측 국가는 원본 학위 국가와 대조하며 충돌 시 사람이 검토하더라도 잘못된 원본 국가를 그대로 확정할 수 없다. 수정된 원본 스냅샷이 필요하다.

버전 2→3→4 또는 3→4 마이그레이션은 기존 논문·연결·검토·배치·관측의 키와 행을 유지한다. 버전 4에서는 공급자 외래키 등록부를 사용해, 이후 실제 기록 형식이 추가되면 `migrate`를 다시 실행해 등록할 수 있다. 트랜잭션 안에서 외래키 검사를 통과한 경우에만 새 버전을 확정하며 실패 시 스키마 변경을 포함해 되돌린다. 버전 3에서 이미 가져온 동일 입력은 새 기관 관측 필드를 소급해서 만들지 않고 기존 배치로 인식한다. 새 입력에는 강화된 기관 검사를 적용한다.
...
# 공개 상세 페이지에서 실제 관측한 기록. OpenAPI 호출과 별도 경로다.
python3 backend/scripts/thesis_registry.py import-riss-public \
  --database "$PRIVATE_DIRECTORY/researcher-identity.sqlite" \
  --candidates-json "$PRIVATE_DIRECTORY/riss-public-details.json"

python3 backend/scripts/thesis_registry.py summary \
  --database "$PRIVATE_DIRECTORY/researcher-identity.sqlite"
```

RISS OpenAPI 후보를 기존 학위에 연결할 때는 원본 연구자 ID·익명 ID·기관·연도·국가·이름을 대조한다. 여러 스냅샷에 동일한 학위가 있으면 `--source-snapshot-sha256`으로 대상을 지정해야 하며 임의로 최신 행을 선택하지 않는다. API 수집기는 국내 `stype=id`, 해외 `stype=od`를 사용한다. RISS 키가 없는 상태에서 OpenAPI 실조회나 API 응답을 이용한 학과 검증을 했다고 기록하지 않는다.
...
## 2026-09-10 2차 대학 도서관·RISS 공개검색 적재

공식 대학 도서관 10곳과 RISS 공개 상세페이지의 관측 자료를 비공개 DB에 추가했다. 새 연결 921건을 저장했고, 처음 논문 후보가 연결된 학위는 861개다. 연결 자료가 있는 학위는 695개에서 1,556개로 늘었다. 자동 확정이나 원본 학위·기관·연도 수정은 하지 않았다.
...
출처 간 학위 중복을 제외하면 이번 수집분은 874개 학위에 해당한다. 같은 서지의 재사용을 제외한 신규 공급자별 서지 레코드는 913개이며, 여러 출처가 같은 실제 논문을 가리킬 수 있다. 모든 추가 연결에 실제 원문의 학과 정보와 서지 링크를 보존했고, 101개 연결에는 DOI도 보존했다. 880개 연결은 발행연도가 기존 학위연도와 같고, 41개는 후보 허용 범위인 ±1년이다.

서울대는 이전 미조회 대상 556개를 조회하고, 표시된 결과 중 처음 제한했던 상세자료 14개도 추가 확인했다. 연세대·고려대의 페이지를 넘는 검색 결과 182개를 추가 점검했다. RISS는 API 키 없이 공개 웹페이지에서 10초 간격으로 65개 학위를 조회했다. RISS 전체 대상 전수검색을 완료했다는 의미는 아니다.

한양대·서강대·건국대의 도서관 경로는 결과 오류·페이지 오류·방화벽 때문에 완료하지 못했고 RISS로 일부 보완했다. 서울과기대로 정규화된 18개 항목은 원본 학교명과 모두 충돌하여 추정 연결하지 않았다. 원본 저자·기관·연도와 맞지 않는 자료, 석사학위, 지도교수만 일치하는 자료는 보류했다.

DB 백업 사본과 실제 DB에서 마이그레이션·적재·중복 재입력·무결성·외래키 검사를 통과했다. 원본 행과 관측 증거를 보존했으며, 다른 연구자 ID가 같은 논문을 가리키는 것이 확인된 기존 후보 1개만 충돌 상태로 바뀌었다. 전체 연결 상태는 후보 1,755건·충돌 27건·확정 0건이다. 공개 데이터·다운로드 파일 8개는 해시가 같고, 비공개 DB와 수집 자료는 저장소 밖에 유지했다. Python 테스트 120개가 통과했다.
