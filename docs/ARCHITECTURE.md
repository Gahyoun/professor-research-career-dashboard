...
연구자 식별자와 졸업논문 검토는 별도 비공개 SQLite에서 관리한다. `thesis_registry.py`는 기존 식별자 DB를 버전 3으로 확장하며, 출처 스냅샷별 박사학위·공급자별 논문·후보 연결·누적 검토 이력과 도서관 관측 자료를 나눈다. `build_thesis_seed.py`는 기존 OpenAlex 자료에서 국내·해외 논문 후보를 생성하고, 발급키로 수집한 RISS 후보는 `import-riss`, KAIST·서울대학교 공식 도서관의 서지 후보는 `import-library`로 추가한다. 도서관 후보는 저장된 학위 기준값과 실제 저자·박사학위 표기·기관·연도를 대조하고 출처 자료의 해시와 수집 시각을 보존한다. 자동 입력은 확정 상태를 만들지 않으며 국가·기관·연도 충돌은 검토 대상으로 남긴다. 이 DB는 웹과 공개 릴리스에 직접 연결하지 않는다. [컬럼 및 실행 절차](THESIS_LINKAGE.md)
