      ...
      {rows.map((row, index) => {
        ...
        return (
          <div className="career-row" key={`${row.stage}-${row.institution}-${index}`}>
            <div className="career-label"><span className={`stage-dot ${row.stage}`} /><span><b>{institutionDisplayName(row.institution)}</b><small>{row.evidence_basis}</small></span>{row.is_estimated && <em>추정</em>}{row.is_institution_successor && <em>기관 승계</em>}</div>
  ...
  return (
    <div className={`app ${contrast ? 'high-contrast' : ''}`} style={{ fontSize: `${zoom}%` }}>
      ...
      <main id="main" tabIndex={-1} className={`container ${viewMode === 'constellation' || viewMode === 'sankey' || viewMode === 'institutions' ? 'atlas-container' : ''}`}>
        ...
        {viewMode === 'constellation' ? <Constellation initialSelectedId={graphSelectionId} professors={dashboard.professors} names={names} releaseYear={dashboard.meta.release_year} onSelect={chooseProfessor}/> : viewMode === 'institutions' ? <TemporalInstitutionsPage professors={dashboard.professors} releaseYear={dashboard.meta.release_year}/> : viewMode === 'sankey' ? <SankeyPage key={names ? 'unlocked' : 'locked'} professors={dashboard.professors} names={names} releaseYear={dashboard.meta.release_year} onSelect={chooseProfessor}/> : <>
        ...
        <section className="filter-panel" aria-labelledby="filter-title"><div className="panel-heading"><div><span className="step">01</span><h2 id="filter-title">조회 조건</h2></div><button className="reset" type="button" onClick={() => { setFilters(emptyFilters); setSelectedId(''); setGroupPhd(''); setGroupCountry(''); setMinimumImpact(0); setMinimumPapers(0); setMinimumJournals(0); }}>초기화</button></div>
          <div className="filter-grid">
            ...
            <label>현재 재직기관<select value={filters.institution} onChange={e => updateFilter('institution', e.target.value)}><option value="">전체 기관</option>{availableInstitutions.map(v => <option key={v} value={v}>{institutionDisplayName(v)}</option>)}</select></label>
            <label>연구자 · 현재 재직기관<select value={selected?.id || ''} onChange={e => { setSelectedId(e.target.value); setViewMode('individual'); }}><option value="">교수 선택</option>{filtered.map(p => <option key={p.id} value={p.id}>{labelFor(p)} · {p.current_institution ? institutionDisplayName(p.current_institution) : '현재기관 미상'} · {subjectLabels[p.subject] || p.subject}</option>)}</select></label>
