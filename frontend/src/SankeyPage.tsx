...
function nodeText(label: string, n: number, width: number) {
  const suffix = ` · ${count(n)}`;
  const measure = (s: string) => [...s].reduce((sum, char) => sum + (/[^\u0000-\u024f]/.test(char) ? 18 : /[MW]/.test(char) ? 15 : char === ' ' ? 5 : 10), 0);
  if (measure(label + suffix) <= width) return label + suffix;
  let trimmed = label;
  while (trimmed.length && measure(trimmed + '…' + suffix) > width) trimmed = trimmed.slice(0, -1);
...
export default function SankeyPage({ professors, names, onSelect, releaseYear }: Props) {
  ...
  const selectedPeople = useMemo(() => [...selectedIds].map(id => professorMap.get(id)!).filter(Boolean).filter(p => {
    ...
    return !query || p.id.toLowerCase().includes(query) || (names?.[p.id] ?? '').toLowerCase().includes(query);
  ...
  return <section className="sankey-page" aria-labelledby="sankey-title">
    ...
    <div className="sk-controls" aria-label="학력 흐름 필터">
      ...
      <label><span>학부 출신</span><select value={origin} onChange={e => { setOrigin(e.target.value as Origin | 'all'); resetSelection(); }}><option value="all">전체 출신</option>{Object.entries(ORIGIN_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
    ...
    <section className="sk-chart-card" aria-labelledby="sk-chart-title">
      ...
      <ul className="sk-legend" aria-label="학부 출신별 색상 범례">{Object.entries(ORIGIN_LABELS).map(([key, label]) => <li key={key}><i style={{ backgroundColor: ORIGIN_COLORS[key as Origin] }} />{label}</li>)}<li className="sk-legend-self"><i />진한 흐름: 학부 모교 재직</li></ul>
      {data.count === 0 ? <div className="sk-empty"><strong>선택한 조건에 맞는 완비 경로가 없습니다.</strong><p>연구 분야 또는 학부 출신 필터를 조정해 주세요.</p></div> : <>
        ...
        <div className="sk-chart-scroll" tabIndex={0} aria-label="학력 흐름 도표. 좌우 스크롤 가능">
          ...
          <svg className="sk-svg" viewBox={`0 0 ${layout.width} ${layout.height}`} width={layout.width} height={layout.height} aria-labelledby="sk-svg-title sk-svg-description">
            ...
            {[226, 706, 1186].map(x => <line key={x} x1={x} x2={x} y1={14} y2={layout.height - 14} stroke="#eef1f4" strokeDasharray="2 5" />)}
            ...
            <g>{layout.nodes.map(node => {
              ...
              const text = nodeText(node.label, node.count, first ? 202 : 188);
              return <g key={node.id} className={`sk-node ${selected ? 'is-active' : ''}`} role="button" tabIndex={0} aria-label={`${info.label}, ${count(node.count)}명. 선택하여 경로 보기`} aria-pressed={selected > 0} onClick={() => choose(info)} onKeyDown={event => keyChoose(event, info)} onFocus={() => setHover(info)} onBlur={() => setHover(null)} onMouseEnter={() => setHover(info)} onMouseLeave={() => setHover(null)}>
                ...
                <rect className="sk-label-bg" x={first ? 3 : node.x + 18} y={node.labelY - 12} width={first ? 205 : 194} height={24} rx={2} fill="#fff" fillOpacity={0.94} />
                <text x={x} y={node.labelY + 6} textAnchor={first ? 'end' : 'start'} fill={focused && !selected ? '#6d7882' : '#1e2124'} fontWeight={selected ? 700 : 400}>{text}</text>
              </g>;
            })}</g>
          </svg>
        </div>
    ...
    {selection && selectedIds.size > 0 && <section className="sk-people" aria-labelledby="sk-people-title">
      ...
      <div className="sk-people-tools"><label><span>연구자 찾기</span><input type="search" value={search} onChange={e => { setSearch(e.target.value); setPeoplePage(0); }} placeholder={names ? '이름 또는 익명 ID' : '익명 ID 입력'} /></label><p>{names ? '연구자를 선택하면 개인 경력과 시공간적으로 유사한 연구자를 볼 수 있습니다.' : '비밀번호로 이름을 공개하기 전에는 익명 ID만 표시합니다.'}</p></div>
