...
import { careerSubjectLabels, lifetimePeriod, lifetimeStageColors, lifetimeStageLabels, lifetimeUnitLabel } from './lifetimePresentation';
...
export default function TrajectoryPanel({ lifetime, stageFilter, selectedGroupId, labelFor, institutionFor, subjectFor, onSelect, onSelectGroup }: {
  ...
  return <section className="trajectory-panel" aria-labelledby="trajectory-title">
    ...
    <p>선택 연구자와 명부의 분야가 같은 구성원만 비교합니다. 단계의 전체 기간 중 겹친 시점을 구성원별로 표시하므로 집단 전체가 동시에 재학·재직했다는 의미는 아닙니다.</p>
    {groups.map(group => <article key={group.id} className={`lifetime-evidence-group ${selectedGroupId === group.id ? 'active' : ''}`} style={{ '--stage-color': lifetimeStageColors[group.stage] } as CSSProperties}>
      <div className="lifetime-evidence-heading"><div><span className="lifetime-phase-label"><i aria-hidden="true"/>{lifetimeStageLabels[group.stage]} · {lifetimePeriod(group.startYear, group.endYear)}</span><h3>{lifetimeUnitLabel(group)}</h3></div><button onClick={() => onSelectGroup(group.id)} aria-pressed={selectedGroupId === group.id}>{selectedGroupId === group.id ? '집단 강조 해제' : '지도에서 이 집단 강조'}</button></div>
      <dl className="lifetime-group-facts"><div><dt>연결 조건</dt><dd>{group.condition}</dd></div><div><dt>선택 연구자의 기간</dt><dd>{lifetimePeriod(group.startYear, group.endYear)}{selectedIntervals.get(group.id)?.estimated ? ' · 추정' : ''}</dd></div><div><dt>분야와 학과 근거</dt><dd>명부의 {careerSubjectLabels[group.subject || ''] || '미상'} 분야로 제한{group.stage !== 'current' && ' · 과거 학과 확인과 별개'}<br/>{group.stage === 'current' ? '현직 세부 학과명과 학과 추정 여부로 나누지 않습니다.' : group.stage === 'postdoc' ? '같은 기관·명부 분야·활동 기간을 함께 비교합니다.' : group.inferredDepartment ? '과거 학과: 논문 소속에서 추정한 학과 포함' : '과거 학과: 해당 시기의 기록 사용'}</dd></div><div><dt>구성원</dt><dd>{group.members.length}명 · 선택 연구자 포함</dd></div></dl>
