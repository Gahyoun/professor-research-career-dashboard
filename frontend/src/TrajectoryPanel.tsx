...
export default function TrajectoryPanel({ lifetime, stageFilter, selectedGroupId, labelFor, institutionFor, subjectFor, onSelect, onSelectGroup }: {
  ...
  const groups = stages.flatMap(stage => stage.groups).filter(group => !selectedGroupId || group.id === selectedGroupId);
  ...
  return <section className="trajectory-panel" aria-labelledby="trajectory-title">
    <div className="atlas-section-title"><div><span className="eyebrow">GROUP MEMBERSHIP EVIDENCE</span><h2 id="trajectory-title">집단 구성원과 겹친 기간</h2><p className="atlas-current-identity">{labelFor(lifetime.selectedId)} · {institutionFor(lifetime.selectedId)}</p></div><span>{groups.length}개 집단</span></div>
    ...
    {groups.map(group => <article key={group.id} className={`lifetime-evidence-group ${selectedGroupId === group.id ? 'active' : ''}`} style={{ '--stage-color': lifetimeStageColors[group.stage] } as CSSProperties}>
      <div className="lifetime-evidence-heading"><div><span className="lifetime-phase-label"><i aria-hidden="true"/>{lifetimeStageLabels[group.stage]} · {lifetimePeriod(group.startYear, group.endYear)}</span><h3>{group.institution}{group.department ? ` · ${group.department}` : ''}</h3></div><button onClick={() => onSelectGroup(group.id)} aria-pressed={selectedGroupId === group.id}>{selectedGroupId === group.id ? '집단 전체로 돌아가기' : '지도에서 이 집단 보기'}</button></div>
      <dl className="lifetime-group-facts"><div><dt>연결 조건</dt><dd>{group.condition}</dd></div><div><dt>선택 연구자의 기간</dt><dd>{lifetimePeriod(group.startYear, group.endYear)}{selectedIntervals.get(group.id)?.estimated ? ' · 추정' : ''}</dd></div><div><dt>학과 근거</dt><dd>{group.stage === 'postdoc' ? '기관 단위 비교 · 학과 조건 없음' : group.inferredDepartment ? '논문 소속에서 추정한 학과 포함' : '기록된 학과'}</dd></div><div><dt>구성원</dt><dd>{group.members.length}명 · 선택 연구자 포함</dd></div></dl>
