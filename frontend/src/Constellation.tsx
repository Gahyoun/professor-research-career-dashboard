...
import { lifetimePeriod, lifetimeStageColors, lifetimeStageLabels, lifetimeUnitLabel } from './lifetimePresentation';
...
const edgeDisplayLabel = (edge: Hyperedge) => edge.institution
  ? edge.lifetimeStage ? `${lifetimeStageLabels[edge.lifetimeStage]} · ${lifetimeUnitLabel({ ...edge, institution: edge.institution })}`
...
export default function Constellation({ professors, names, releaseYear, onSelect, initialSelectedId = '' }: {
  ...
  const lifetimeIndex = useMemo(() => createLifetimeIndex(comparisonRecords, { releaseYear, includeEstimated, estimatedYears: years, includeInferredDepartments, stages: enabledStages }, allRecords), [comparisonRecords, allRecords, releaseYear, includeEstimated, years, includeInferredDepartments, enabledStages]);
  ...
  useEffect(() => {
    ...
    worker.postMessage({
      ...
      records: graphRecords, selectedId: scopeSelectedId, preparedLifetime,
    ...
    return () => { active = false; worker.terminate(); };
  }, [graphRecords, scopeSelectedId, preparedLifetime, lifetimeStage, enabledStages, releaseYear, spatial, temporal, level, includeEstimated, includeInferredDepartments, years, balance, spacing, cohortEnabled, bachelorTime, requestKey]);
  ...
  return <section className="atlas" aria-labelledby="atlas-title">
    <div className="atlas-heading"><div><div className="atlas-kicker">K–STEM ATLAS <span>/ {releaseYear}</span></div><h1 id="atlas-title">Career trajectory hypergraph</h1><p>연구자별로 명부의 분야가 같은 박사·포닥·현직 연결을 탐색합니다. 각 단계의 기관과 기간, 필요한 과거 학과 근거를 함께 비교합니다.</p></div><div className="atlas-count"><strong>{professors.length.toLocaleString()}</strong><span>전체 교수 <i>·</i> 현재 그래프 {filtered.length.toLocaleString()}명</span></div></div>
    ...
    {scope === 'researcher' && <section className="lifetime-explorer" aria-labelledby="lifetime-explorer-title">
      ...
      <p className="lifetime-rule">하나의 하이퍼엣지 = 선택한 경력 단계·기관·기간에서 명부의 분야가 같은 집단. 한 연구자는 여러 집단에 동시에 포함됩니다. 그 기간 중 한 번이라도 겹친 동료와 근거가 있는 재직 교수를 함께 묶습니다. 구성원마다 실제로 겹친 연도는 아래 근거 목록에서 확인합니다.</p>
    ...
    <div className="atlas-workspace">
      ...
      <aside className="atlas-sidebar" aria-label="하이퍼그래프 탐색 조건">
        ...
        <div className="atlas-sidebar-section"><h2>{scope === 'researcher' ? '기간과 근거' : '연결 조건'} <span>02</span></h2>
          ...
          {scope === 'researcher' && <p className="atlas-quiet">모든 경력 단계에서 명부의 같은 분야(수학·물리·화학·생물)만 연결합니다. 박사·첫 조교수는 같은 학교와 과거 학과, 포닥은 같은 기관, 현직은 같은 학교를 비교합니다. 해당 시기의 겹침도 필요합니다.</p>}
          ...
          <label className="atlas-check"><input type="checkbox" checked={includeInferredDepartments} onChange={e => recompute(() => setIncludeInferredDepartments(e.target.checked))}/>논문 소속에서 추정한 학과 포함</label><p className="atlas-quiet">{scope === 'researcher' ? '학과 추정 설정은 박사·첫 조교수에 적용합니다. 포닥은 같은 기관·명부 분야로 비교하며, 현직은 같은 학교·명부 분야에서 세부 학과명이나 추정 여부로 나누지 않습니다. 명부 분야는 과거 학과를 확인한 근거가 아닙니다.' : '학교별 보기에서는 해당 학위의 학과 근거에 적용합니다. 공간·시기 조건은 위 선택에 따르며, 같은 명부 분야 제한은 연구자별 경력 집단에 적용됩니다.'}</p>
        </div>
    ...
    <details className="atlas-method"><summary>집단과 임베딩의 계산 기준</summary><p>연구자별 경력 집단은 모든 단계에서 명부의 분야가 같은 연구자만 포함합니다. 재직 교수와 연결할 때도 같은 기준을 적용합니다. 이 분야는 공개 명부의 수학·물리·화학·생물 분류이며, 과거의 전공이나 학과를 독립적으로 확인한 근거는 아닙니다. 분야 미상은 연결하지 않습니다.</p><p>박사·첫 조교수는 국내외 모두 같은 학교·과거 학과·명부 분야와 겹친 기간이 필요합니다. 포닥은 같은 기관·명부 분야와 겹친 활동 기간을 함께 비교합니다. 현직은 관측된 같은 학교·명부 분야로 묶으며 세부 학과명 차이나 학과 누락으로 나누지 않습니다. 과거 학과를 현재 학과로 대신 채우지 않습니다. 논문 소속에서 추정한 학과를 포함한 경우는 각 집단과 구성원 근거에 별도로 표시합니다.</p><p>연구자별 하이퍼엣지는 선택 연구자의 단계·기관·기간을 기준으로 만든 집합입니다. 박사과정 집단은 재학 기간이 겹친 동료와 재직 근거가 있는 교수를 함께 포함합니다. 연도마다 잘게 나누지 않으며, 구성원별 겹친 기간을 보존합니다. 첫 조교수 단계는 일반 교수 경력의 순번에서 추측하지 않습니다. 현직은 공개 자료에서 관측된 시점을 사용합니다. 학교별 보기는 별도의 출신학교·학과와 추정 시기 조건을 사용하며, 분야 버튼으로 표시 대상을 좁힐 수 있습니다.</p><p>매끈한 윤곽은 집단의 표시입니다. 윤곽 안에 들어온 점도 구성원이 아닐 수 있으며, 실제 소속은 점 둘레의 단계 색상과 구성원 목록으로 확인합니다. 동일 구성원의 집단도 별도 윤곽으로 표시하며, 집단 선택은 강조만 바꿉니다. 연구자별 화면에서는 소속된 모든 집단 중심의 가중 평균으로 배치하고 노드 충돌을 줄입니다. 시간 순서는 같은 소속 조합 안에서만 반영합니다. 학교별 화면은 집합 크기와 중첩을 보정한 스펙트럴 임베딩을 여러 초기값에서 계산합니다. 전역 최적해를 보장하지 않으며, 거리는 실제 교류 강도가 아닙니다.</p><p>계산은 공개 익명 데이터만으로 브라우저에서 이루어집니다. 비밀번호 해제 후 이름은 화면 표시에만 사용하며 계산 작업과 브라우저 저장소에 전달하지 않습니다.</p></details>
  </section>;
}
