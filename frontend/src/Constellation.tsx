...
export default function Constellation({ professors, names, releaseYear, onSelect, initialSelectedId = '' }: {
  ...
  return <section className="atlas" aria-labelledby="atlas-title">
    <div className="atlas-heading"><div><div className="atlas-kicker">K–STEM ATLAS <span>/ {releaseYear}</span></div><h1 id="atlas-title">Career trajectory hypergraph</h1><p>박사·포닥·현직의 경력 집단을 탐색합니다. 현직은 같은 학교·계열의 재직 연구자를 함께 묶습니다.</p></div><div className="atlas-count"><strong>{professors.length.toLocaleString()}</strong><span>전체 교수 <i>·</i> 현재 그래프 {filtered.length.toLocaleString()}명</span></div></div>
    ...
    <div className="atlas-workspace">
      ...
      <aside className="atlas-sidebar" aria-label="하이퍼그래프 탐색 조건">
        ...
        <div className="atlas-sidebar-section"><h2>{scope === 'researcher' ? '기간과 근거' : '연결 조건'} <span>02</span></h2>
          ...
          {scope === 'researcher' && <p className="atlas-quiet">현직은 같은 학교 + 같은 계열(수학·물리·화학·생물)로 묶습니다. 박사·첫 조교수는 학교+학과, 포닥은 학교·기관을 비교합니다.</p>}
          ...
          <label className="atlas-check"><input type="checkbox" checked={includeInferredDepartments} onChange={e => recompute(() => setIncludeInferredDepartments(e.target.checked))}/>논문 소속에서 추정한 학과 포함</label><p className="atlas-quiet">학과 추정은 박사·첫 조교수 연결에 적용합니다. 현직은 명부의 계열을 사용하므로 학과 추정 설정과 관계없이 같은 학교·계열을 묶습니다. 재직 시점의 근거는 필요합니다.</p>
        </div>
    ...
    <details className="atlas-method"><summary>집단과 임베딩의 계산 기준</summary><p>현직은 관측된 같은 학교·계열로 묶으며 세부 학과명 차이나 학과 누락으로 나누지 않습니다. 박사·첫 조교수는 국내외 모두 학교+학과, 포닥은 기관 단위로 비교합니다. 과거 학과를 현재 학과로 대신 채우지 않습니다. 논문 소속에서 추정한 학과를 포함한 경우는 각 집단과 구성원 근거에 별도로 표시합니다.</p><p>연구자별 하이퍼엣지는 선택 연구자의 단계·기관·기간을 기준으로 만든 집합입니다. 박사과정 집단은 재학 기간이 겹친 동료와 재직 근거가 있는 교수를 함께 포함합니다. 연도마다 잘게 나누지 않으며, 구성원별 겹친 기간을 보존합니다. 첫 조교수 단계는 일반 교수 경력의 순번에서 추측하지 않습니다. 현직은 공개 자료에서 관측된 시점을 사용합니다.</p><p>매끈한 윤곽은 집단의 표시입니다. 윤곽 안에 들어온 점도 구성원이 아닐 수 있으며, 실제 소속은 점 둘레의 단계 색상과 구성원 목록으로 확인합니다. 동일 구성원의 집단도 별도 윤곽으로 표시하며, 집단 선택은 강조만 바꿉니다. 연구자별 화면에서는 소속된 모든 집단 중심의 가중 평균으로 배치하고 노드 충돌을 줄입니다. 시간 순서는 같은 소속 조합 안에서만 반영합니다. 학교별 화면은 집합 크기와 중첩을 보정한 스펙트럴 임베딩을 여러 초기값에서 계산합니다. 전역 최적해를 보장하지 않으며, 거리는 실제 교류 강도가 아닙니다.</p><p>계산은 공개 익명 데이터만으로 브라우저에서 이루어집니다. 비밀번호 해제 후 이름은 화면 표시에만 사용하며 계산 작업과 브라우저 저장소에 전달하지 않습니다.</p></details>
  </section>;
}
