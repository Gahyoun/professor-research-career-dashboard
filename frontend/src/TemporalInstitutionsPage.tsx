...
function Distribution({ title, rows, denominator, empty, labelFor }: { title: string; rows: CountShare[]; denominator: string; empty: string; labelFor?: (label: string) => string }) {
  const [expanded, setExpanded] = useState(false);
  const visible = expanded ? rows : rows.slice(0, 12);
  return <article className="iw-distribution"><h3>{title}</h3><p className="iw-denominator">{denominator}</p>{rows.length ? <><div className="iw-distribution-rows">{visible.map(row => <div className="iw-distribution-row" key={row.label}><span>{labelFor ? labelFor(row.label) : row.label}</span><strong>{count(row.count)}명</strong><small>{share(row.share)}</small><i aria-hidden="true" style={{ width: `${Math.max(0, Math.min(100, (row.share ?? 0) * 100))}%` }}/></div>)}</div>{rows.length > 12 && <button className="iw-text-button" onClick={() => setExpanded(value => !value)} aria-expanded={expanded}>{expanded ? '상위 12개만 보기' : `전체 ${rows.length}개 항목 보기`}</button>}</> : <p className="iw-empty-inline">{empty}</p>}</article>;
}
...
function MovementDetails({ movement, currentCount, year, termLabel }: { movement: InstitutionMovement; currentCount: number; year: number; termLabel: string }) {
  ...
  return <>
    ...
    <div className="iw-two-columns"><Distribution title="관측 유입의 출발 기관" rows={movement.sources} labelFor={institutionDisplayName} denominator={`분모: 통합 승계를 제외한 단일 기관 간 관측 유입 ${count(movement.movesIn)}명`} empty="조건에 해당하는 소속 변경이 없습니다."/><Distribution title="관측 유출의 도착 기관" rows={movement.destinations} labelFor={institutionDisplayName} denominator={`분모: 통합 승계를 제외한 단일 기관 간 관측 유출 ${count(movement.movesOut)}명`} empty="조건에 해당하는 소속 변경이 없습니다."/></div>
  </>;
}
...
export default function TemporalInstitutionsPage({ professors, releaseYear }: { professors: Professor[]; releaseYear: number }) {
  ...
  return <section className="workforce-page" aria-labelledby="iw-title">
    ...
    <div className="iw-controls" aria-label="통계 조회 조건">
      ...
      <label className="iw-institution-select"><span>기관 · 국가</span><select value={institution?.id || ''} disabled={!data.institutions.length} onChange={e => setInstitutionId(e.target.value)}>{data.institutions.length ? data.institutions.map(row => <option key={row.id} value={row.id}>{institutionDisplayName(row.label)} · {countryLabel(row.country)}</option>) : <option value="">선택 분야의 관측 기관 없음</option>}</select></label>
    </div>
    ...
    {institution && point ? <>
      <section className="iw-institution-heading"><div><h2>{institutionDisplayName(institution.label)}</h2><p>{selectedYear}년 {termLabel} · {subjectLabel} · {countryLabel(institution.country)}</p></div><span className="iw-scope-chip">실제 학기 명부 기준</span></section>
    ...
    <section className="iw-method" aria-labelledby="iw-method-title"><h2 id="iw-method-title">계산과 해석 범위</h2><ul><li>학기별 교수 명부의 실제 관측만 사용합니다. 이력서·논문 소속·추정 경력 구간으로 학기별 인원을 보충하지 않습니다.</li><li>소속 변경은 인접 연도 같은 학기의 관측 분야 범위가 같을 때만 비교합니다. 두 시점 모두 단일 기관이 확인된 이동과 대학 통합 승계, 복수·미해소 기관, 미관측을 구분합니다. 같은 분야 범위여도 기관별 수집 완전성을 보장하지 않습니다.</li><li>{institutionDisplayName('강원대학교')}·{institutionDisplayName('경상국립대학교')}의 기관 집계 ID는 이름이 바뀌어도 유지합니다. {institutionDisplayName('강릉원주대학교')}·{institutionDisplayName('경남과학기술대학교')}는 각각 통합 시행연도부터 승계 기관에 합치며, 원래 기준 기관에 계속 있던 사람을 신규 승계 인원으로 세지 않습니다.</li><li>출신 구성은 선택 명부 전체를 분모로 삼습니다. 미상 항목을 빼서 비중을 높이지 않으며, 학위연도 요약의 유효 인원은 따로 표시합니다. 복수 기관에 관측된 사람은 각 기관에서 세므로 기관별 비중의 합은 100%를 넘을 수 있습니다.</li></ul><p>이 자료는 과거 교수 전체를 무작위로 뽑은 표본이 아닙니다. 명부의 선택과 누락으로 기관 전체에 일반화할 수 없는 차이가 생길 수 있습니다. 이러한 선택·포괄 범위의 제한은 <a href="https://www150.statcan.gc.ca/n1/edu/power-pouvoir/ch13/nonprob/5214898-eng.htm" target="_blank" rel="noreferrer">Statistics Canada의 비확률 표본 설명</a>을 참고하세요. 이 페이지는 자동 유의확률이나 신뢰구간을 제시하지 않습니다.</p></section>
  </section>;
}
